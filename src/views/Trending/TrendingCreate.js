...
const TrendingCreate = () => {
  const navigate=useNavigate() 
  const {getAlltreadings,isTreading,setIsTrending}=useTrending()
  const [treadingData, setTreadingData] = useState({
    content: "",
    image: "",
    coverImagePreview: "",
  });
  const { token,getallepisode } = useSeries();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLoading(true);
      const res = await axios.post(`/api/create/trading/${id}`, form, {
        headers: {
