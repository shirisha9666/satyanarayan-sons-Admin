...
export const HomeCollectionProvider = ({ children }) => {
  ...
  const handlegetAllData = async (page = 1, itemPerPage, bannertype) => {
    try {
      setLoading(true);
      const response = await axios.get("/api/trending/get/all", {
        params: {
          page,
          limit: itemPerPage,
  ...
  const handleDelete = async (id) => {
    try {
      setBannerId(id);
      let resp = await axios.delete(`/api/trending/delete/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleOneBanner = async (id) => {
    try {
      setViewBannerId(id);
      let resp = await axios.get(`/api/trending/get/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
export const useHomeCollection = () => useContext(HomeCollectionContext);
