...
const EditProductCollection = () => {
  ...
  const getProductCollection = async () => {
    axios
      .get(`/api/productCollection/getOne/${id}`, {
        headers: {
      ...
      .then((res) => {
        // console.log(res.data?.productCollection?.image)
        // if (res.data?.productCollection?.image) {
        //     res.data?.productCollection?.image.map(item => {
        //     })

        // }

        // setImagesPreview(res.data?.productCollection?.image);
        // setImage preview for each image in productCollection
        // res.data?.productCollection?.image?.map((item) => {
        ...
        setData((prev) => ({
          ...prev,
          ...res.data?.productCollection,
          image: [],
          imageURL: res.data?.productCollection?.image?.url,
        }));
      })
      .catch((err) => {});
  };
  ...
  const handleSubmit = () => {
    ...
    axios
      .put(`/api/productCollection/update/${data._id}`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      .then((res) => {
        swal({
          title: "Updated",
          text: "productCollection Updated successfully!",
          icon: "success",
          button: "ok",
        });
        setLoading(false);
        navigate("/product-collections", { replace: true });
      })
      .catch((err) => {
        setLoading(false);
          ...
          <div className="card h-100">
            <div className="card-body px-5">
              ...
              {imagesPreview.length > 0 && (
                <div
                  id="createproductCollectionFormImage"
